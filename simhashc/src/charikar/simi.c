...
charikar_hash64( const char *data, long data_length)
{
	...
	int noStopWords = sizeof(stopwords) / sizeof(std::string);
	for (int is = 0; is < noStopWords; is++){
	  stopSet.insert(stopwords[is]);
	}
	...
	while ((get_next_token( &data, data_end, &token_start, &token_length))) {
		...
		if (!stopWord(std::string( token_start, token_length))){
